...
def main():
    ...
    xmin, xmax = 0., 1.
    ymin, ymax = 0., 1.
    dx, dy = 5e-3, 5e-3   # on report at the moment (Feb 10)
    ...
    for n in range(0, n_max+1):   # for is faster than while
        ...
        # converged?
        u_res = np.sqrt(np.sum((u - u_) ** 2)) / np.sqrt(np.sum(u_ ** 2))   # L2 norm
        v_res = np.sqrt(np.sum((v - v_) ** 2)) / np.sqrt(np.sum(v_ ** 2))
        ...
        if n % 1000 == 0:
            ...
            u_Ghia = [1., 0.65928, 0.57492, 0.51117, 0.46604, 0.33304, 0.18719, 0.05702, -0.0608, -0.10648, -0.27805, -0.38289, -0.2973, -0.2222, -0.20196, -0.18109, 0.]
            ...
            v_Ghia = [0., -0.21388, -0.27669, -0.33714, -0.39188, -0.515, -0.42665, -0.31966, 0.02526, 0.32235, 0.33075, 0.37095, 0.32627, 0.30353, 0.29012, 0.27485, 0.]
